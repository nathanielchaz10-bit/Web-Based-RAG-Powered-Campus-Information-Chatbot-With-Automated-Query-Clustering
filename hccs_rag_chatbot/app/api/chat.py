...
def guest_chat(
    ...
    query = QueryLog(
        ...
        is_guest=True,     # flag it so the dashboard can label guest inquiries
        timestamp=datetime.utcnow(),
        response_time_ms=result["response_time_ms"],
        is_valid=True,
        sentiment=sentiment,
...
def _session_title(text: str | None) -> str:
    """Sidebar title from a session's first question: trimmed and capitalized.
    The sidebar span truncates with a CSS ellipsis, so we only cap length (no
    manual '...') and don't LLM-summarize -- cheap and budget-free."""
    text = (text or "").strip()
    if not text:
        return "New Conversation"
    return (text[0].upper() + text[1:])[:60]
...
def get_user_sessions(
    ...
    for s in sessions:
        ...
        title = _session_title(first_query.query_text if first_query else None)
...
def delete_session(
    ...
    """Delete a chat session. Its QueryLog rows are kept (session_id NULLed) so
    they still feed admin dashboards / query clustering."""
    session = db.query(ChatSession).filter(
        ChatSession.session_id == session_id,
        ChatSession.user_id == user.user_id
    ).first()
...
def delete_all_sessions(
    ...
    """Delete every chat session belonging to the current user. Backs the
    "Clear all chat history" button in Settings → Privacy & Data.

    Uses db.delete() per-row (not a bulk query.delete()) so SQLAlchemy's default
    relationship handling NULLs each session's QueryLog.session_id instead of
    deleting the rows. The student's conversations disappear, but the underlying
    queries/responses are kept (de-identified) for admin dashboards and query
    clustering.
    """
